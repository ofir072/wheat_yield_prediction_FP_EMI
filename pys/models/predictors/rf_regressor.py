...
# RandomForestRegressor initial parameter grid
...
def perform_search(x_train_selected, y_train, x_val_selected, y_val, model, param_grid, model_name, run_detail, k,
    ...
    print(f"{run_detail} - Best Params: {search.best_params_}")

    return results, best_regressor, search.best_params_
...
def evaluate_model_with_fs_optimized(x_train, y_train, x_val, y_val, feature_selector, k, model, initial_param_grid,
    ...
    initial_results, best_regressor, best_initial_params = perform_search(x_train_selected, y_train, x_val_selected,
    ...
    refined_results, best_regressor, best_refined_params = perform_search(x_train_selected, y_train, x_val_selected,
    ...
    final_results, best_regressor, _ = perform_search(x_train_selected, y_train, x_val_selected, y_val, model,
                                                      further_refined_rf_param_grid, model_name, "Final Grid Search", k,
                                                      feature_selector.__class__.__name__, search_type='grid')

    return initial_results + refined_results + final_results, best_regressor, x_train_selected, x_val_selected
...
def model_with_fs_optimized(df, model, initial_param_grid, model_name, results_dir, dataset_name):
    ...
    for feature_selector in feature_selectors:
        for k in ks:
            ...
            if model_results[-1]['R2'] > best_r2_score:
                best_r2_score = model_results[-1]['R2']
        ...
        with pd.ExcelWriter(os.path.join(results_dir, f'results_with_config_{dataset_name}.xlsx')) as writer:
        ...
        if "reduced" in filename:
            ...
            best_model, x_val_selected, y_val = model_with_fs_optimized(df_to_train,
                                                                        RandomForestRegressor(
                                                                            random_state=config["KFold random_state"]),
                                                                        rf_initial_param_grid, "RandomForestRegressor",
