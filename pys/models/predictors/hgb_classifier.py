...
def run_hgb_all_models():
    ...
    for filename in os.listdir(data_dir):
        if "reduced" in filename:
            if "אחוזון" in filename:
                dataset_path = os.path.join(data_dir, filename)
                dataset_name = "HistGradientBoostingClassifier_" + os.path.splitext(filename)[0]
                ...
                best_model, x_val_selected, y_val = model_with_fs_optimized(df_to_train, HistGradientBoostingClassifier(
                                                                            random_state=42),
                                                                            "HistGradientBoostingClassifier",
                                                                            results_dir, dataset_name, param_grid,
                                                                            'ROC AUC')
                calculate_and_save_feature_importance(best_model, x_val_selected, y_val, results_dir,
                                                      dataset_name, 'predict_proba', 'brute')
            else:
